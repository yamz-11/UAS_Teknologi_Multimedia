			...
			this.Continue = function (oArgs)
			{
				if(this.nStartTime==0)
				{
					...
					if(CLO[this.oParams.sTargetId]==null)
					{
						...
						{
							var jxObjectNode = CL.jxModule.find("object[id='" + this.oParams.sTargetId + "']");
							if(jxObjectNode.length!=0)
							{
								CLO[this.oParams.sTargetId] = new CLObject({ xNode: jxObjectNode[0], oAppendTo: CL.oBoard, oParentSlide: CLS[CLZ.sCurrentSlideId], oParentFrame: CLF[CLZ.sCurrentFrameId], sSlideId: CLZ.sCurrentSlideId, sFrameId: CLZ.sCurrentFrameId, bMaster: false });
								CLO[this.oParams.sTargetId].Render(); // Call Render after creating object to proceed with Constructor
			...
			this.Continue = function (oArgs)
			{
				if(this.nStartTime == 0)
				{
					...
					switch(this.oParams.sType)
					{
						...
						case "score":
						{
							var sCurrentScore = CL.SCO.GetObjectiveScore({ sId: this.oParams.sObjectiveId, sSrcId: this.oParams.sSourceId });
						...
						if(oCurObj!=null)
						{
							...
							if(this.oParams.sColorBorder!=null)
							{
								...
								switch(oCurObj.sType)
								{
									...
									default:
									{
										var jWrapper = $(oCurObj.div).children(":first");
										jWrapper.css({ "border-color": this.oParams.sColorBorder });
										if(jWrapper.css("border-style")=="none" || jWrapper.css("border-style")=="")
										{
											jWrapper.css({ "border-style": "solid" });
										}
										break;
									}
								}
...
{	// CLFrame methods
	CLFrame.prototype.Start = function (oArgs)
	{
		...
		oThis.Show({ bResetObjects: (oArgs!=null && oArgs.bResetFrame==true) });
	...
	CLFrame.prototype.Show = function (oArgs)
	{
		...
		{
			CLZ.sCurrentFrameId = oThis.sId;
			oThis.jxObjects.filter("[type='v_rapid_007']").each(function ()
			{
				CLO[this.getAttribute("id")].Show();
			});
			var bReset = (oArgs!=null && oArgs.bResetObjects==true);
			for(var i=0; i<oThis.jxObjects.length; i++)
			{
				if(oThis.jxObjects[i].getAttribute("type")=="v_rapid_007")
				{
					continue;
				}
				sObjectId = oThis.jxObjects[i].getAttribute("id");
...
{	// CLMaster constructor
	...
	for(var i=0; i<this.jxFrames.length; i++)
	{
		...
		if(CLF[sFrameId]==null)
		{
			CLF[sFrameId] = new CLFrame({ xNode: this.jxFrames[i], oParentSlide: this, bMaster: true }).Hide({ bMaster: true });
		}
	}
	return this;
};
...
{	// CLObject constructor
	...
	this.oParentSlide = oArgs.oParentSlide;
	this.oParentFrame = oArgs.oParentFrame;
	...
	if(this.sType=="SHAPE")
	{
		var jShapeBox = this.div.find(".cl-shape-box");
		if(jShapeBox.length!=0)
		{
			var nOffsetX = parseFloat(jShapeBox.attr("cl-offset-x"));
			var nOffsetY = parseFloat(jShapeBox.attr("cl-offset-y"));
			if(!isNaN(nOffsetX))
			{
				if(nOffsetX>0)
				{
					jShapeBox.css({ "left": ("-" +  nOffsetX + "px") });
				}
			}
			if(!isNaN(nOffsetY))
			{
...
{	// CLObject methods
	CLObject.prototype.Reset = function (oArgs)
	{
		var oThis = this;
		oThis.current = JSON.parse(JSON.stringify(oThis.initial));
		this.div.css(
		{
			"left": this.initial.x + "px",
			"top": this.initial.y + "px",
	...
	CLObject.prototype.Show = function (oArgs)
	{
		...
		if(oThis.iFlickerTimeout!=null && oThis.iFlickerTimeout!=0)
		{
			clearTimeout(oThis.iFlickerTimeout);
		}
		if(oArgs!=null)
		{
	...
	CLObject.prototype.Hide = function (oArgs)
	{
		...
		if(oArgs!=null)
		{
			if(oArgs.bActionCall==true)
			{
				...
				if(oArgs.bUseTransition==false)
				{
					if((CL.sCSSPrefix=="-moz-" || CL.sCSSPrefix=="-ms-") && (oThis.sType=="BASIC" || this.sType=="SHAPE"))
					{
						this.iFlickerTimeout = setTimeout(function () { oThis.div.hide() }, 100); //trying to override FF/IE flickering bug on images
					}
					else
					{
						$(oThis.div).hide();
		...
		else
		{
			...
			else
			{
				if((CL.sCSSPrefix=="-moz-" || CL.sCSSPrefix=="-ms-") && (oThis.sType=="BASIC" || this.sType=="SHAPE"))
				{
					this.iFlickerTimeout = setTimeout(function () { oThis.div.hide() }, 100) ; // trying to override FF flickering bug on images
				}
				else
				{
					$(oThis.div).hide();
...
{ // CLModulePath
	...
	{ // sSlideId
		...
		if(oArgs.sSlideId=="")
		{
			if(CL.bAlternativePath && oArgs.bUserAction)
			{
				alert(CL.oPath.oChapters[sCurrentChapter].sMsg);
			}
			return false;
		}
		...
		if(sCurrentChapter!=sTargetChapter && $.inArray(oArgs.sSlideId, CLZ.aVisited)==-1)
		{
...
var CL =
{
	...
	Init: function (oArgs)
	{
		...
		function Mobile()
		{
			...
			if(nua.toLowerCase().indexOf("webtutor") > -1) // WT mobile app only
			{
				$(CL.oBoard).parent().css({ "left": "0", "top": "0", "margin": "0" });
				CL.pz = new PinchZoom($(CL.oBoard).parent()[0],
				{
					tapZoomFactor: 3,
					zoomOutFactor: 1.3,
				...
				CL.pz.AfterResize = function (e)
				{
					var jContainer = $(this.el);
					var sMatrix = jContainer.css("transform");
					if(sMatrix.indexOf("matrix(")!=-1)
					{
						sMatrix = sMatrix.split("(")[1].split(")")[0];
						var aValues = sMatrix.split(",");
						var nRatio = parseFloat(aValues[0]);
						aValues[4] = 0.5*(jContainer.parent().width() - nRatio*jContainer.width());
						aValues[5] = 0.5*(jContainer.parent().height() - nRatio*jContainer.height());
						jContainer.css({ "transform": "matrix(" + aValues.join(",") + ")" });
					}
					return false;
				...
				{
					if(CL.pz.zoomFactor>1)
					{
						CL.pz.scaleTo(1, CL.pz.getCurrentZoomCenter());
					}
					$(window).one("resize", function (event)
					{
						setTimeout(function () { CL.pz.AfterResize(event); },100);
					});
				};
		...
		CL.Mask({ on: true });
		var sXParams = "";
		try { sXParams = (g_sWTParams==null ? "" : "?a=" + g_sWTParams); } catch(e) {}
		$.ajax({
			type: "GET",
			url: "splash.xml" + sXParams,
			async: true,
			dataType: "xml",
			complete: function ()
			{
	...
	Navigation:
	{
		...
		{
			var bAllowed = true;
			var sTarget = (oArgs.sTarget!=null) ? oArgs.sTarget : oArgs.sTargetType;
			var sDir = (oArgs.sTargetDir!=null) ? oArgs.sTargetDir : oArgs.sDir;
			switch(sTarget)
			{
				case "slide":
				{
					var sTargetSlideId = ((sDir!=null) ? CL.oPath._GetTargetSlideId({ sTarget: sDir }) : oArgs.sTargetId);
					bAllowed = CL.oPath._TargetAllowed({ sSlideId: sTargetSlideId, bUserAction: (oArgs.bUserAction==true) });
					break;
				}
				case "frame":
				{
		...
		Exists: function (oArgs)
		{
			...
			switch(sTarget)
			{
				case "slide":
				{
					if( (sDir=="next" && CLZ.sCurrentSlideId==CL.oPath.aSlideOrder[CL.oPath.aSlideOrder.length-1]) || (sDir=="prev" && CLZ.sCurrentSlideId==CL.oPath.aSlideOrder[0]))
					{
						bExists = false;
					}
					break;
				...
				case "frame":
				{
					if(sDir!=null)
					{
						switch(sDir)
						{
							case "next":
							{
								bExists = !CLF[CLZ.sCurrentFrameId].bIsLast;
								break;
							}
							case "prev":
							{
								bExists = !CLF[CLZ.sCurrentFrameId].bIsFirst;
								break;
							}
						}
					}
				...
				switch(oArgs.sTargetType)
				{
					...
					case "frame":
					{
						...
						if(sFrameId!="")
						{
							...
							{
								CLF[sFrameId].Start({ bResetFrame: (CLZ.sCurrentFrameId==sFrameId) });
							}
						}
						break;
					}
	...
	SCO:
	{
		_HasModuleCompletionRule: function (oArgs)
		{
			var bHasRule = false;
			var sObjId;
			...
			{
				sObjId = axRules[i].getAttribute("objective");
				if(CLJ[sObjId]!=null)
				{
					if(+CLJ[sObjId].bModule==1)
					{
						if(axRules[i].getAttribute("cs")=="c")
						{
							bHasRule = true;
							break;
						}
		...
		_HasModuleSuccessRule: function (oArgs)
		{
			var bHasRule = false;
			var sObjId;
			var sSS;
		...
		ApplyRules: function (oArgs)
		{
			...
			var bCheckSS = false;
			var sObjId;
			var bModuleCSChanged = false;
			var bModuleSSChanged = false;
			for(var i=0; i<axRules.length; i++)
			{
				jxRule = $(axRules[i]);
				...
				if(CLJ[sObjId]!=null)
				{
					...
					if(bCheckSS || bCheckCS)
					{
						if(+CLJ[sObjId].bModule==1)
						{
							bModuleCSChanged = bCheckCS;
							bModuleSSChanged = bCheckSS;
						}
						var bSuccess = false;
			...
			if(CLLMS.bXAPI==true)
			{
				if(bModuleCSChanged)
				{
					CLLMS.FireEvent({ sEvent: "completionchanged" });
				}
				if(bModuleSSChanged)
				{
					CLLMS.FireEvent({ sEvent: "successchanged" });
				}
			}
			return true;
		},
		...
		GetObjectiveScore: function (oArgs)
		{
			...
			if((oArgs.sSrcId==null) || (oArgs.sSrcId=="") || (oArgs.sSrcId=="_*_"))
			{
				return +CLJ[oArgs.sId].nRawScore;
			}
			else
			{
				if(CLJ[oArgs.sId].oSrc[oArgs.sSrcId]!=null)
				{
		...
		SetObjectiveScore: function (oArgs)
		{
			...
			var nScore = Math.round(+oArgs.nScore*100)/100;
			if(oArgs.bReplace==true)
			{
			...
			else
			{
				...
				nScore = +CLJ[oArgs.sId].oSrc[sSrcId].nRawScore + nScore;
				nScore = Math.round(+nScore*100)/100;
				CLJ[oArgs.sId].oSrc[sSrcId].nRawScore = nScore;
			}
			var nTotal = 0;
			for(var sKey in CLJ[oArgs.sId].oSrc)
...
//		oArgs.pid = argobj.pid;
	...
	Start: function (oArgs)
	{
		function GetReady()
		{
			...
			var jxCXSettings = CL.jxModule.children("cx_settings");
			if(jxCXSettings.length!=0)
			{
				if(jxCXSettings.text()!="")
				{
					var oCXSettings = JSON.parse(jxCXSettings.text());
					if(typeof oCXSettings == "object")
					{
						var sStyleString = ".cx-font-size-base { font-size: " + oCXSettings.nFontSize + "px; }\n";
						sStyleString += ".cx-font-size-small { font-size: " + (oCXSettings.nFontSizeSmall/100) + "em; }\n";
						sStyleString += ".cx-font-size-large { font-size: " + (oCXSettings.nFontSizeLarge/100) + "em; }\n";
						sStyleString += ".cx-font-size-header { font-size: " + (oCXSettings.nFontSizeHeader/100) + "em; }\n";
						sStyleString += ".cx-font-size-footer { font-size: " + (oCXSettings.nFontSizeFooter/100) + "em; }\n";
						sStyleString += ".cx-font-family-base { font-family: " + oCXSettings.sFontFamily + "; }\n";
						sStyleString += ".cx-color-base { color: " + oCXSettings.sColorFont + "; }\n";
						sStyleString += ".cx-color-header { color: " + oCXSettings.sColorHeader + "; }\n";
						sStyleString += ".cx-color-footer { color: " + oCXSettings.sColorFooter + "; }\n";
						sStyleString += ".cx-color-btn, .cxx-tab-btn .cx-color-tab { background-color: " + oCXSettings.sColorBtn + "; }\n";
						sStyleString += ".cx-color-btn[disabled] { background-color: " + oCXSettings.sColorBtnDisabled + "; }\n";
						sStyleString += ".cx-color-btn:not([disabled]):hover, .cxx-tabs-tab .cxx-tab-btn[cl-state='selected'] .cx-color-tab, .cxx-tabs-tab .cxx-tab-btn:not([cl-state='selected']):hover .cx-color-tab { background-color: " + oCXSettings.sColorBtnHover + "; }\n";
						sStyleString += ".cxx-tabs-accordion .cxx-tab-item[cl-state='selected'] .cx-color-tab, .cxx-tabs-accordion .cxx-tab-item:not([cl-state='selected']):hover .cx-color-tab { background-color: " + oCXSettings.sColorBtnHover + "; }\n";
						sStyleString += ".cxx-tab-btn .cx-color-arrow { border-color: transparent transparent transparent " + oCXSettings.sColorBtn + "; }\n";
						sStyleString += ".cxx-tab-btn[cl-state='selected'] .cx-color-arrow, .cxx-tab-btn:not([cl-state='selected']):hover .cx-color-arrow { border-color: transparent transparent transparent " + oCXSettings.sColorBtnHover + "; }\n";
						sStyleString += ".cxx-q .cxx-var[cl-type='choice'] .cxx-radio-spot-inner.cx-color-spot { fill: " + oCXSettings.sColorBtnHover + "; }\n";
						sStyleString += ".cxx-q .cxx-var[cl-type='select'] .cxx-check-mark.cx-color-spot { fill: " + oCXSettings.sColorBtnHover + "; }\n";
						sStyleString += ".cx-color-success { color: " + oCXSettings.sColorSuccess + "; }\n";
						sStyleString += ".cx-color-failure { color: " + oCXSettings.sColorFailure + "; }\n";
						sStyleString += ".cxx-q .cxx-var[cl-type='choice'][cl-fb='correct'] .cxx-radio-spot-outer { fill: " + oCXSettings.sColorSuccess + "; }\n";
						sStyleString += ".cxx-q .cxx-var[cl-type='choice'][cl-fb='incorrect'] .cxx-radio-spot-outer { fill: " + oCXSettings.sColorFailure + "; }\n"
						sStyleString += ".cxx-q .cxx-var[cl-type='select'][cl-fb='correct'] .cxx-var-spot-body { background-color: " + oCXSettings.sColorSuccess + "; }\n";
						sStyleString += ".cxx-q .cxx-var[cl-type='select'][cl-fb='incorrect'] .cxx-var-spot-body { background-color: " + oCXSettings.sColorFailure + "; }\n"
						sStyleString += "a:link, a:active { color: " + oCXSettings.sColorLink + "; }\n"
						sStyleString += "a:visited { color: " + oCXSettings.sColorLinkVisited + "; }"
						var jStyle = $("head").children("style");
						if(jStyle.length==0)
						{
							jStyle = $(document.createElement("style")).appendTo("head");
						}
						jStyle.html(sStyleString);
					}
				}
			}
			CL.SCO.Create();
			if(jxCXSettings.length!=0)
			{
				// counting max score
				var nMaxScore = 0;
				CL.jxModule.find("object[type='clx_question']").each(function ()
				{
					var jxParam = CL.jxParams.children("param[objectid='" + this.getAttribute("id") + "']");
					var nScore = parseFloat(jxParam.children("q_score").text());
					if(!isNaN(nScore)) nMaxScore += nScore;
				});
				CLJ["total"].nMax = nMaxScore;
			}
			CLLMS.Init();
			CLLMS.Load();
